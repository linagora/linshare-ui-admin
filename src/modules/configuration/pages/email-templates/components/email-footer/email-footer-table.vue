<template>
  <div class="email-footer-table">
    <a-table
      key="uuid"
      class="email-footer-table__table"
      :data-source="mailFooterByPage"
      :pagination="false"
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'domain'">
          <span v-if="!checkingEmailFootersDomainAuthorized(record.domain)">{{ record.domainName }}</span>
          <router-link
        ...
        <template v-if="column.key === 'action'">
          <a-dropdown overlay-class-name="email-footer-table__dropdown" placement="bottomRight" :trigger="['click']">
            ...
            <template #overlay>
              <a-menu>
                ...
                <a-menu-item
                  ...
                  class="delete"
                  @click="onDeleteMailFooter(record)"
                >
...
const rowSelection = computed(() => ({
  checkStrictly: false,
  selectedRowKeys: selectedMailFooters.value?.map((item) => item.uuid) ?? [],
  onSelect: (record: MailFooter, selected: boolean, selectedRows: MailFooter[]) => {
    selectedMailFooters.value = selectedRows;
  },
  onSelectAll: (selected: boolean, selectedRows: MailFooter[], changeRows: MailFooter[]) => {
    selectedMailFooters.value = selectedRows;
  },
  onChange: (selected: boolean, selectedRows: MailFooter[], changeRows: MailFooter[]) => {
    selectedMailFooters.value = selectedRows;
  },
}));

const mailFooterByPage = computed(() => {
...
const columns = computed(() => [
  {
    ...
    key: 'description',
    sorter: (a: MailFooter, b: MailFooter) => a.description?.localeCompare(b.description),
  },
  {
    title: t('EMAIL_TEMPLATES.READ_ONLY'),
    key: 'readonly',
    sorter: (a: MailFooter, b: MailFooter) => Number(b.readonly) - Number(a.readonly),
  },
  {
    title: t('GENERAL.DOMAIN'),
    key: 'domain',
    sorter: (a: MailFooter, b: MailFooter) => a.domainName?.localeCompare(b.domainName),
  },
  {
    title: t('GENERAL.CREATION_DATE'),
    key: 'creationDate',
    sorter: (a: MailFooter, b: MailFooter) => (a.creationDate || 0) - (b.creationDate || 0),
  },
  {
    title: t('GENERAL.MODIFICATION_DATE'),
    key: 'modificationDate',
    sorter: (a: MailFooter, b: MailFooter) => (a.modificationDate || 0) - (b.modificationDate || 0),
    defaultSortOrder: 'descend',
  },
  {
    title: t('EMAIL_TEMPLATES.VISIBILITY'),
    key: 'visible',
    sorter: (a: MailFooter, b: MailFooter) => Number(b.visible) - Number(a.visible),
  },
  {
