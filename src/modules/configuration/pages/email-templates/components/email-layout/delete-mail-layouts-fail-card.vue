<template>
  <a-card flat :bordered="false" class="delete-mail-layouts-fail-card">
    <span class="delete-mail-layouts-fail-card__icon">
      <warning-icon width="36" height="36"></warning-icon>
    </span>
    <div class="delete-mail-layouts-fail-card__content">
      <strong>{{ $t('EMAIL_TEMPLATES.DELETE_LAYOUT_MODAL.DELETE_FAILURE') }}</strong>
      <span>{{ deleteMessage }}</span>
    </div>
    <div class="delete-mail-layouts-fail-card__actions">
...
// composable
const { t } = useI18n();
const { modal } = useEmailTemplatesLayout();
//props & emits
...
const deleteMessage = computed(() => {
  if (modal.multipleDeleteResponse?.total === 1) {
    if (modal.multipleDeleteResponse?.totalAssignCases === 1) {
      return t('EMAIL_TEMPLATES.DELETE_LAYOUT_MODAL.DELETE_ERROR_ASSIGNED');
    } else if (modal.multipleDeleteResponse?.totalUnAuthoCases === 1) {
      return t('EMAIL_TEMPLATES.DELETE_LAYOUT_MODAL.DELETE_ERROR_UNAUTHORIZED');
    } else {
      return t('EMAIL_TEMPLATES.DELETE_LAYOUT_MODAL.DELETE_ERROR_ONE');
    }
    ...
    if (modal.multipleDeleteResponse?.totalAssignCases === modal.multipleDeleteResponse?.totalFail) {
      return t('EMAIL_TEMPLATES.DELETE_LAYOUT_MODAL.DELETE_ERROR_ASSIGNED_ALL');
    } else if (modal.multipleDeleteResponse?.totalUnAuthoCases === modal.multipleDeleteResponse?.totalFail) {
      return t('EMAIL_TEMPLATES.DELETE_LAYOUT_MODAL.DELETE_ERROR_UNAUTHORIZED_ALL');
    } else {
    ...
    if (modal?.multipleDeleteResponse?.totalAssignCases && modal?.multipleDeleteResponse?.totalAssignCases > 0) {
      return t('EMAIL_TEMPLATES.DELETE_LAYOUT_MODAL.DELETE_ERROR_ASSIGNED_SOME');
    } else if (
