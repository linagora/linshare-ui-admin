<template>
  <a-card flat :bordered="false" class="delete-mime-policy-card">
    <span class="delete-mime-policy-card__icon">
      <DeleteIcon width="28" height="28" />
    </span>
    <div class="delete-mime-policy-card__content">
      <strong>{{ $t('MIME_POLICIES.DELETE_MODAL.DELETE_MODAL_TITLE') }}</strong>
      <span>{{ $t('MIME_POLICIES.DELETE_MODAL.DELETE_MODAL_SUBTITLE') }}</span>
    </div>
    <div class="delete-mime-policy-card__actions">
...
// composable
const { loading, handleDeleteMimePolicy } = useMimesPolicies();
...
async function onConfirmDelete() {
  const result = await handleDeleteMimePolicy();
  if (result) {
    emits('refresh');
    emits('close');
  }
