<template>
  <a-card flat :bordered="false" class="delete-contact-card">
    ...
    <div class="delete-contact-card__content">
      <strong>{{ $t('CONTACT_LIST.DELETE_CONTACT_MODAL.DELETE_MODAL_TITLE') }}</strong>
      <span>{{ $t('CONTACT_LIST.DELETE_CONTACT_MODAL.DELETE_MODAL_SUBTITLE') }}</span>
    </div>
...
// composable
const { t } = useI18n();
const { loading, handleDeleteContactList, activeContactList } = useContactList();
...
async function onConfirmDelete() {
  const result = await handleDeleteContactList(activeContactList.value);
  if (result) {
