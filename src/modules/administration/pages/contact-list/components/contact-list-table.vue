    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'identifier'">
          <div class="user-infos" @click="onEditContactList(record)">
            <div>
              <a-avatar shape="circle" :size="46" class="profile-avatar">
                <span> {{ record.identifier.charAt(0) }}</span>
              </a-avatar>
            </div>
            <div class="account-name">
              <strong :title="record.identifier">{{ record.identifier }}</strong>
              <br />
...
const { status, filteredListByPage, selectedContactLists, onEditContactList } = useContactList();
...
const columns = computed(() => [
  ...
  },
  {
    ...
    align: 'center',
    dataIndex: ['domain'],
    sorter: (a: Contact, b: Contact) => a.domainLabel.localeCompare(b.domainLabel),
    key: 'domain',
  },
  {
