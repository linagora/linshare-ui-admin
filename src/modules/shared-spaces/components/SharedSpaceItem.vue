<template>
  <div class="list-item">
    <a-list-item @click="goToDetails">
      <a-list-item-meta>
        <template #avatar>
          <WorkgroupIcon v-if="data.nodeType === SHARED_SPACE_TYPE.WORKGROUP" fill="#0372B3" />

          <WorkspaceIcon v-else fill="#0372B3" />
        </template>

        <template #title>
          <span class="list-item__name">{{ data.name }}</span>
          <span v-if="data.parentName" class="list-item__parent">@{{ data.parentName }}</span>
        </template>
